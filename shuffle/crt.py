...
def crt_generalized(remainders: List[int], moduli: List[int]) -> Tuple[int, int]:

    if len(remainders) != len(moduli):

        return None

    firstRemainder: int = remainders[0]
    firstModulus: int = moduli[0]

    for i in range(1, len(remainders)):

        secondRemainder = remainders[i]
        secondModulus = moduli[i]

        d, _, _ = EEA(firstModulus, secondModulus)

        if firstRemainder % d != secondRemainder % d:

            print(f"{firstRemainder} and {secondRemainder} Not congruent modulo d = {d}")

            return None

        # by definition, d divides both first- and second modulus, so we can safely cast to int
        _, s, t = EEA(int(firstModulus/d), int(secondModulus/d))

        mod = int(firstModulus / d*secondModulus)

        x = (firstRemainder*(int(secondModulus/d))*t + secondRemainder*(int(firstModulus/d))*s) % mod
    ...
    return firstRemainder, firstModulus
